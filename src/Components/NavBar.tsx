...
const NavBar: React.FC<INavBar> = (): any => {
  ...
  const UpdateProfile = async () => {
    try {
      ...
      const { data } = await axios.post(`/x/updateuser`, {
        id: String(user.id),
        email,
        username,
      ...
      if (data?.error) {
        ...
        onClose();
        return dispatch(
          AddUser({ email: data.email, username: data.username, ID: data.ID })
        );
      }
    } catch (error) {
      toast({
  ...
  const logOut = async () => {
    try {
      ...
      throw new Error("Cant log you out.");
    } catch (error) {
        ...
        {!user.email && !user.username ? (
          <Box my="2">
            ...
            <Link
              ...
              to="/login"
            >
              <Button colorScheme="facebook" size="ls" variant="solid" p="2">
                LOGIN
              </Button>
            </Link>
            <Link
                ...
                <DrawerBody>
                  ...
                  <FormControl id="newPassword">
                    ...
                    {!password.length && (
                      <>
                        {" "}
                        <Text as="span" fontSize="xs" mx="2">
                          Leave it empty if dont want to update password.
                        </Text>{" "}
                        <br />{" "}
                      </>
                    )}
                  ...
                  {user.posts.length && (
                    <>
                      ...
                      <Box my="5">
                        ...
                        >
                          Visit Your Posts{" "}
                          <ExternalLinkIcon color="yellow.500" />
                        </Link>
                      </Box>
                    </>
                  )}
